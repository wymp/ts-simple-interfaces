...
describe("SimplePubSubAmqp", () => {
  ...
  const amqpConnect = (opts: unknown) => {
  ...
  test("handles complex publish correctly", async () => {
    ...
    expect(JSON.stringify(mockCnx.channel!.calls.publish[0][3].headers)).toBe(JSON.stringify({ one: 1 }));
  ...
  test("rebinds and retries on error during publish", async () => {
    ...
    // Now try to publish
    const msg = { my: "message", num: 1 };
    await expect(ps.publish(`my-exchange`, msg, { routingKey: "message.created" })).resolves.toBeUndefined();
